...
export async function createDraftQuote({
  ...
  // Fixed items — differ by shop
  const LM_FIXED_ITEMS = [
  ...
  const DEFAULT_FIXED_ITEMS = [
    'Calibration Identification Report',
    'Post Collision Safety Inspection 1',
    'Post-Scan',
  ]

  // Detect L-M shop: "L-M" must be a standalone token in the name,
  // not just any word containing the letters l and m together (e.g. "elm", "film").
  const isLMJob = [shop, customerName].some((n) => {
    if (!n) return false
    // Split on whitespace and commas, strip punctuation from each token, check for exact "lm"
    return n.toLowerCase().split(/[\s,;]+/).some(
      (token) => token.replace(/[^a-z]/g, '') === 'lm'
    )
  })
  const fixedNames = isLMJob ? LM_FIXED_ITEMS : DEFAULT_FIXED_ITEMS
  console.log(`[zoho] Shop type: ${isLMJob ? 'L-M' : 'Standard'} → using ${fixedNames[1]}`)

  const unmatchedItems = []

  function buildLineItem(name, description) {
    const match = findBestMatch(name, exactMap, allItems)
    if (match) {
      console.log(`[zoho] ✓ "${name}" → "${match.matchedName}" (score ${match.score.toFixed(2)})`)
      return { item_id: match.item_id, description: description || '', quantity: 1 }
    } else {
      unmatchedItems.push(name)
      console.warn(`[zoho] ✗ No match for: "${name}"`)
      return { name, description: description || '', quantity: 1, rate: 0.0 }
    }
  }

  // Fixed items first, then calibrations
  const fixedLineItems = fixedNames.map((name) => buildLineItem(name, ''))
  const calLineItems = calibrations.map((cal) => {
    ...
    return buildLineItem(cal.calibration_name, parts.join('\n'))
  })
  const lineItems = [...fixedLineItems, ...calLineItems]

  if (unmatchedItems.length > 0) {
    console.warn('[zoho] Unmatched items:', unmatchedItems)
  }

  // 3. Notes — VIN, insurer, claim only (vehicle fields go in custom fields)
  const notesLines = [
    vin     ? `VIN: ${vin}`         : null,
