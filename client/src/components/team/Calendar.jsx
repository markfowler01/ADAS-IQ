...
export default function CalendarTab({ user, onOpen }) {
  ...
  const load = () => { setErr(''); j(`/api/people/calendar?from=${start}&to=${end}`).then(setData).catch(e => setErr(e.message)) }
  useEffect(() => { load() }, [start]) // eslint-disable-line react-hooks/exhaustive-deps
  // One tap from the calendar (D): mark the check-in done and open the
  // person's card so the 1:1 / review gets logged right there.
  async function logIt(e) { try { await j(`/api/people/checkin/${e.member_id}/done`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ key: e.checkin }) }); load(); onOpen && onOpen(e.member_id) } catch (err) { alert(err.message) } }
          ...
          <div className="rounded-xl overflow-hidden" style={{ border: '1px solid #e8e4e0', backgroundColor: 'white' }}>
            {Object.entries(days).sort().map(([d, evs]) => (
              <div key={d} className="flex gap-3 px-3 py-2" style={{ borderTop: '1px solid #f3f3f3', backgroundColor: d === today ? '#fff5f0' : 'white' }}>
                ...
                <div className="flex-1 space-y-0.5">{evs.map((e, i) => <div key={i} className="text-sm flex items-center gap-2 flex-wrap" style={{ color: TYPES[e.type]?.[2] || '#333', fontWeight: e.mine ? 700 : 500 }}>{e.title}{owner && e.checkin && e.member_id && <button onClick={() => logIt(e)} className="text-[11px] font-bold rounded-full px-2.5 py-0.5" style={{ backgroundColor: 'white', color: '#2563eb', border: '1px solid #2563eb' }}>✓ Done — log it</button>}</div>)}</div>
              </div>
            ))}
          </div>
        </div>
