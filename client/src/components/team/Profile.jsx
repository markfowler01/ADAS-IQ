...
export default function ProfileDrawer({ m: card, members, user, onClose, onEdit, onChanged }) {
  ...
  const [linkForm, setLinkForm] = useState(null)      // {mode:'full'|'catchup'|'revoke', phone}
  const [linkResult, setLinkResult] = useState('')
  const [docName, setDocName] = useState(null)        // {file} waiting for a name
  const [signoff, setSignoff] = useState(null)        // {key,label} waiting for a note
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" style={{ backgroundColor: 'rgba(0,0,0,.5)' }} onClick={onClose}>
      <div className="bg-white w-full sm:max-w-lg rounded-t-2xl sm:rounded-2xl p-4 max-h-[94vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        {canSee && (
          <Section title="Documents" right={<div className="flex gap-1">{m.workdrive_folder_url ? <a href={m.workdrive_folder_url} target="_blank" rel="noreferrer" className="text-[11px] font-bold rounded-full px-2.5 py-1" style={{ backgroundColor: '#dbeafe', color: BLUE }}>📁 Open folder</a> : owner && <Small tone="orange" onClick={() => j(`/api/people/folder/${m.id}`, { method: 'POST' }).then(load).catch(e => alert(e.message))}>📁 Create folder</Small>}<label className="text-[11px] font-bold rounded-full px-2.5 py-1 cursor-pointer" style={{ backgroundColor: 'white', color: ORANGE, border: `1px solid ${ORANGE}` }}>{busy ? '⏫ …' : '⏫ Upload'}<input type="file" accept="image/*,.pdf" hidden onChange={e => { const f = e.target.files?.[0]; e.target.value = ''; if (f) setDocName({ file: f, name: f.name.replace(/\.[^.]+$/, '') }) }} /></label>{owner && <Small tone="orange" onClick={() => { setAdding('document'); setForm({}) }}>＋ link</Small>}</div>}>
            {docName && <div className="rounded-lg p-2 mb-2 flex gap-2 flex-wrap items-center" style={{ backgroundColor: '#fff5f0', border: `1px solid ${ORANGE}` }}><span className="text-xs" style={{ color: '#888' }}>{docName.file.name}</span><input value={docName.name} onChange={e => setDocName(d => ({ ...d, name: e.target.value }))} placeholder="What is this? (W-4, license, signed contract…)" className="text-sm rounded-lg px-2 py-1.5 flex-1 min-w-[160px]" style={inp} /><button onClick={async () => { setBusy(true); try { const fd = new FormData(); fd.append('file', docName.file, docName.file.name); fd.append('name', docName.name || docName.file.name); const r = await apiFetch(`${API_BASE}/api/people/folder/${m.id}/upload`, { method: 'POST', body: fd }); const d = await r.json().catch(() => ({})); if (!r.ok) throw new Error(d.error || `HTTP ${r.status}`); setDocName(null); await load() } catch (err) { alert(err.message) } finally { setBusy(false) } }} disabled={busy || !docName.name.trim()} className="text-xs font-bold rounded-lg px-3 py-1.5 text-white" style={{ backgroundColor: GREEN }}>{busy ? '…' : 'Upload'}</button><button onClick={() => setDocName(null)} className="text-xs font-semibold px-2" style={{ color: '#888' }}>cancel</button></div>}
            {(m.documents || []).length === 0 && <div className="text-xs" style={{ color: '#999' }}>{me && !owner ? 'Nothing here yet. Upload a photo of your ID, W-4, or direct deposit form and it goes straight to your personnel folder.' : 'No documents yet. Upload from the phone, or paste WorkDrive links: W-4, I-9, direct deposit, signed handbook, contract.'}</div>}
        ...
        {p?.ladder && (
          <Section title={`Skills ladder · ${p.ladder.pct}%${p.ladder.complete ? ' · complete' : ''}`} right={owner && p.ladder.complete && m.track === 'apprentice' && (promote
              ? <span className="flex gap-1 items-center"><span className="text-[11px] font-bold" style={{ color: '#1a1a1a' }}>Promote {m.name.split(' ')[0]} to technician?</span><button onClick={() => j(`/api/people/promote/${m.id}`, { method: 'POST' }).then(() => { setPromote(false); load(); onChanged && onChanged() }).catch(e => alert(e.message))} className="text-[11px] font-bold rounded-full px-2.5 py-1 text-white" style={{ backgroundColor: GREEN }}>Yes</button><button onClick={() => setPromote(false)} className="text-[11px] px-1" style={{ color: '#888' }}>no</button></span>
              : <Small tone="orange" onClick={() => setPromote(true)}>🎓 Promote to technician</Small>)}>
              ...
              <div key={r.key} className="py-1.5" style={{ borderBottom: '1px solid #f3f3f3' }}>
                <div className="flex items-center justify-between gap-2 text-sm">
                  ...
                  <span className="flex items-center gap-2"><b className="tabular-nums" style={{ color: r.complete ? GREEN : '#7c3aed' }}>{r.done}/{r.need}</b>
                    {p.can_sign_off && (signoff?.key === r.key
                      ? <span className="flex gap-1 items-center"><input autoFocus value={signoff.note} onChange={e => setSignoff(x => ({ ...x, note: e.target.value }))} placeholder="which car / RO (optional)" className="text-xs rounded-lg px-2 py-1 w-40" style={inp} onKeyDown={e => { if (e.key === 'Escape') setSignoff(null) }} /><button onClick={() => { j(`/api/people/ladder/${m.id}/signoff`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ key: r.key, note: signoff.note }) }).then(() => { setSignoff(null); load() }).catch(e => alert(e.message)) }} className="text-[11px] font-bold rounded-full px-2.5 py-1 text-white" style={{ backgroundColor: GREEN }}>✓ Sign</button><button onClick={() => setSignoff(null)} className="text-[11px] px-1" style={{ color: '#888' }}>×</button></span>
                      : <Small tone="orange" onClick={() => setSignoff({ key: r.key, note: '' })}>＋ sign off</Small>)}
        ...
        {canSee && (
          <Section title="Onboarding link" right={owner && <div className="flex gap-1">{!linkForm && <Small tone="orange" onClick={() => setLinkForm({ mode: 'full', phone: m.personal_phone || m.phone || '' })}>📨 {m.onboarding_invited_at ? 'Resend' : 'Send'} link</Small>}{!linkForm && <Small onClick={() => setLinkForm({ mode: 'catchup', phone: m.personal_phone || m.phone || '' })}>🧩 Catch-up link</Small>}{(m.onboarding_invited_at || m.catchup_invited_at) && !linkForm && <Small tone="red" onClick={() => setLinkForm({ mode: 'revoke' })}>Revoke</Small>}</div>}>
            {linkForm && (
              <div className="rounded-lg p-2.5 mb-2" style={{ backgroundColor: linkForm.mode === 'revoke' ? '#fef2f2' : '#fff5f0', border: `1px solid ${linkForm.mode === 'revoke' ? '#fecaca' : ORANGE}` }}>
                {linkForm.mode === 'revoke' ? (
                  <>
                    <div className="text-xs font-bold mb-2" style={{ color: RED }}>Kill every onboarding link sent to {m.name.split(' ')[0]}? Anyone holding an old link is locked out. You can send a fresh one after.</div>
                    <div className="flex gap-2"><button onClick={async () => { setBusy(true); try { await j(`/api/people/onboarding/${m.id}/revoke`, { method: 'POST' }); setLinkForm(null); setLinkResult('Links revoked.'); await load() } catch (e) { alert(e.message) } finally { setBusy(false) } }} disabled={busy} className="text-xs font-bold rounded-lg px-3 py-1.5 text-white" style={{ backgroundColor: RED }}>Yes, revoke</button><button onClick={() => setLinkForm(null)} className="text-xs font-semibold px-2" style={{ color: '#888' }}>cancel</button></div>
                  </>
                ) : (
                  <>
                    <div className="text-xs font-bold mb-1.5" style={{ color: '#1a1a1a' }}>{linkForm.mode === 'catchup' ? '🧩 Catch-up: photo + emergency contact + handbook signature (about 5 minutes)' : '📨 Full onboarding: photo, ID, deposit, handbook, training (about 20 minutes)'}</div>
                    <div className="text-[11px] mb-1.5" style={{ color: '#666' }}>Goes by text and email. Any older link stops working.</div>
                    <div className="flex gap-2 flex-wrap items-center">
                      <input value={linkForm.phone} onChange={e => setLinkForm(f => ({ ...f, phone: e.target.value }))} placeholder="Cell for the text (blank = email only)" className="text-sm rounded-lg px-2 py-1.5 flex-1 min-w-[180px]" style={inp} inputMode="tel" />
                      <button onClick={async () => { setBusy(true); try { const r = await j(`/api/people/onboarding/${m.id}/invite`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ mode: linkForm.mode, personal_phone: linkForm.phone || undefined }) }); setLinkForm(null); setLinkResult(`Text: ${r.sms ? (r.sms.ok ? 'sent to ' + r.sms.to : 'failed — ' + r.sms.error) : 'no phone'} · Email: ${r.email ? (r.email.ok ? 'sent to ' + r.email.to : 'failed — ' + r.email.error) : 'no email'}`); await load() } catch (e) { alert(e.message) } finally { setBusy(false) } }} disabled={busy} className="text-xs font-bold rounded-lg px-3 py-1.5 text-white" style={{ backgroundColor: GREEN }}>{busy ? 'Sending…' : 'Send'}</button>
                      <button onClick={() => setLinkForm(null)} className="text-xs font-semibold px-2" style={{ color: '#888' }}>cancel</button>
                    </div>
                  </>
                )}
              </div>
            )}
            {linkResult && <div className="text-xs mb-2 px-2 py-1.5 rounded-lg" style={{ backgroundColor: '#dcfce7', color: GREEN }}>{linkResult}</div>}
            <div className="flex gap-1.5 flex-wrap text-[11px] font-bold">
              {[['photo', '📷 photo', !!m.photo_url], ['ids', '🪪 ID + SSN', (m.documents || []).some(x => x.kind === 'dl_front') && (m.documents || []).some(x => x.kind === 'ssn')], ['w4', '🧾 W-4', (m.documents || []).some(x => x.kind === 'w4')], ['mvr', '🚗 driving record', (m.documents || []).some(x => x.kind === 'mvr')], ['deposit', '🏦 direct deposit', !!(m.direct_deposit || m.payout)], ['handbook', '✍️ HR policy', !!m.signatures?.handbook], ['tech_handbook', '📕 tech handbook', !!m.signatures?.tech_handbook], ['contract', '📄 contract', !!m.signatures?.contract], ['training', '🎓 training', !!(p?.member?.training && Object.values(p.member.training).length && Object.values(p.member.training).every(t => t.passed))]].map(([k, l, ok]) => <span key={k} className="px-2 py-0.5 rounded-full" style={ok ? { backgroundColor: '#dcfce7', color: GREEN } : { backgroundColor: '#f5f3f0', color: '#999' }}>{ok ? '✓ ' : ''}{l}</span>)}
            </div>
            ...
            {m.license_expiry && <div className="text-[11px] mt-1" style={{ color: m.license_expiry < new Date().toISOString().slice(0, 10) ? RED : '#888' }}>🪪 License {m.license_expiry < new Date().toISOString().slice(0, 10) ? 'EXPIRED' : 'expires'} {m.license_expiry}{m.license_last4 ? ` · …${m.license_last4}` : ''}{m.mvr_checked_at ? ` · MVR ${m.mvr_checked_at}` : ''}</div>}
            <div className="text-[11px] mt-1" style={{ color: '#888' }}>{m.onboarding_invited_at ? `Link sent ${String(m.onboarding_invited_at).slice(0, 10)}` : ''}{m.catchup_invited_at ? ` · catch-up sent ${String(m.catchup_invited_at).slice(0, 10)}` : ''}{m.onboarding_revoked_at ? ` · revoked ${String(m.onboarding_revoked_at).slice(0, 10)}` : ''}{m.onboarding_completed_at ? ` · ✅ completed ${String(m.onboarding_completed_at).slice(0, 10)}` : ''}</div>
