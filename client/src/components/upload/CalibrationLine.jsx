...
export default function CalibrationLine({ cal, onToggle, price, onField, vehicle }) {
  ...
  const priceEl = price
    ? <span className="text-sm font-bold px-2 py-0.5 rounded tabular-nums" style={price.needs_price ? { backgroundColor: '#fef2f2', color: '#b91c1c' } : { backgroundColor: enabled ? '#dcfce7' : '#f5f3f0', color: enabled ? '#15803d' : '#999' }}>
    ...
    <div style={{ borderTop: '1px solid #f1f5f9', backgroundColor: 'white', opacity: enabled ? 1 : .55 }}>
      ...
      <div className="flex items-center gap-3 px-4" style={{ minHeight: 52 }}>
        ...
        <button type="button" onClick={onToggle} className="flex-1 min-w-0 text-left">
          <div className="text-base font-semibold leading-snug" style={{ color: '#1a1a1a' }}>{calibration_name}</div>
      ...
      {enabled && (
        <div className="px-4 pb-3" onClick={e => e.stopPropagation()}>
          <textarea
            ...
            rows={Math.min(8, Math.max(2, Math.ceil(String(justification || '').length / 110)))}
            className="w-full text-sm rounded-lg px-3 py-2 leading-relaxed"
          ...
          <div className="flex items-center gap-2 mt-1.5 flex-wrap">
            <button type="button" onClick={() => setMetaOpen(o => !o)} className="text-xs font-bold px-2 py-1 rounded-lg" style={{ backgroundColor: '#f5f3f0', color: '#666' }}>
              {metaOpen ? '▾ lines / trigger' : '▸ lines / trigger'}
            </button>
            <button type="button" onClick={aiRewrite} disabled={aiBusy} className="text-xs font-bold px-2.5 py-1 rounded-lg text-white" style={{ backgroundColor: '#7c3aed', opacity: aiBusy ? .6 : 1 }}>
