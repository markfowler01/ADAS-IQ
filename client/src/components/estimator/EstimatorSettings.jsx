    ...
    <Overlay onClose={onClose}>
      ...
      <div className="px-5 py-4 space-y-3 overflow-y-auto" style={{ maxHeight: '72vh' }}>
        ...
        <Panel tone="green" title="Defaults for new estimates">
          ...
          <Row left="Work guaranteed (months · miles)" right={<div className="flex gap-1"><input defaultValue={s.warranty_months ?? 12} onBlur={e => set({ warranty_months: Math.max(0, parseInt(e.target.value, 10) || 0) })} className="px-2 py-1 text-sm w-16 text-right" style={inp} /><input defaultValue={s.warranty_miles ?? 12000} onBlur={e => set({ warranty_miles: Math.max(0, parseInt(String(e.target.value).replace(/[^0-9]/g, ''), 10) || 0) })} className="px-2 py-1 text-sm w-24 text-right" style={inp} /></div>} sub="Printed on estimates and invoices; the service history shows what's still covered." />
        ...
        <Panel tone="orange" title="🔩 PartsTech — parts ordering" right={s.partstech?.connected ? 'connected ✓' : 'not connected'}>
          <div className="px-3 py-2 text-xs" style={{ color: '#666', borderTop: '1px solid #f1f5f9' }}>
            Four values, all pasted by you here (never in chat). <b>User ID + User key</b>: your PartsTech shop account → Settings → API. <b>Partner ID + Partner key</b>: from PartsTech's partner team (PartsTech-Partner-API@oeconnection.com). Keys are stored on the server and never shown again — leave the dots to keep a saved key.
          </div>
          <Row left="Partner ID" right={<input defaultValue={s.partstech?.partner_id || ''} onBlur={e => set({ partstech: { ...(s.partstech || {}), partner_id: e.target.value.trim() } })} className="px-2 py-1 text-sm" style={{ ...inp, width: 200, fontFamily: 'IBM Plex Mono, monospace' }} disabled={!isOwner} />} />
          <Row left="Partner key" right={<input type="password" autoComplete="off" defaultValue={s.partstech?.partner_key || ''} onBlur={e => set({ partstech: { ...(s.partstech || {}), partner_key: e.target.value.trim() } })} className="px-2 py-1 text-sm" style={{ ...inp, width: 200, fontFamily: 'IBM Plex Mono, monospace' }} disabled={!isOwner} />} />
          <Row left="User ID" right={<input defaultValue={s.partstech?.user_id || ''} onBlur={e => set({ partstech: { ...(s.partstech || {}), user_id: e.target.value.trim() } })} className="px-2 py-1 text-sm" style={{ ...inp, width: 200, fontFamily: 'IBM Plex Mono, monospace' }} disabled={!isOwner} />} />
          <Row left="User key" right={<input type="password" autoComplete="off" defaultValue={s.partstech?.user_key || ''} onBlur={e => set({ partstech: { ...(s.partstech || {}), user_key: e.target.value.trim() } })} className="px-2 py-1 text-sm" style={{ ...inp, width: 200, fontFamily: 'IBM Plex Mono, monospace' }} disabled={!isOwner} />} />
        </Panel>
