...
publicRouter.get('/project/:token', async (req, res) => {
  ...
  if (!c) return res.status(404).set('Content-Type', 'text/html; charset=utf-8').send(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Link expired · Absolute ADAS</title><style>body{margin:0;background:#0d0d0d;font-family:Inter,-apple-system,Helvetica,Arial,sans-serif;color:#fff;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:32px 16px}.card{background:#f0ece6;color:#1a1a1a;max-width:520px;width:100%;border-radius:16px;box-shadow:0 12px 40px rgba(0,0,0,.4);padding:36px 30px;border-top:5px solid #CD4419}.brand{font-size:11px;font-weight:800;letter-spacing:.16em;color:#CD4419;text-transform:uppercase;margin-bottom:8px}h1{font-size:24px;margin:0 0 10px;font-weight:900;line-height:1.15}p{color:#444;font-size:15px;line-height:1.55;margin:0 0 12px}a{color:#CD4419;font-weight:700}</style></head><body><div class="card"><div class="brand">Absolute ADAS</div><h1>This project link has expired.</h1><p>Private links stop working once a project's been received or the application was closed out.</p><p>If you still need to send your project, just reply to Mark's email with the Word doc and your video link attached — that works fine too. Or email <a href="mailto:mark@absoluteadas.com">mark@absoluteadas.com</a>.</p></div></body></html>`)
