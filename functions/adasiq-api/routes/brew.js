...
async function notifyMarkOfSignup({ email, name, shop, location, role, total }) {
  ...
  const recipient = process.env.BREW_SIGNUP_NOTIFY_EMAIL || 'brew@absoluteadas.com'
...
async function sendWelcomeEmail({ email, name }) {
  ...
  return sendBroadcast({
    recipients: [email],
    subject: `Welcome to ADAS Brew ☕ + your cheat sheet`,
    html,
    text,
  })
}
...
// GET /api/cron/brew/_send-all-emails?to=brew@absoluteadas.com&secret=...
...
// Posts a recap of the prior month to Cliq #adasbrew + emails brew@absoluteadas.com.
...
function buildSocialCaption(digest) {
  ...
  lines.push('', 'ADAS Brew is published by Absolute ADAS — mobile ADAS calibration in Western Washington. 50,000+ calibrations on the floor.')
