...
router.post('/run', async (req, res) => {
  ...
  try {
    ...
    try { const { maybePhotosOwedDigest } = await import('./jobs.js'); const po = await maybePhotosOwedDigest(req); if (po.fired) console.log('[photos-owed] 6pm list:', po.owed, 'job(s)') } catch (e) { console.log('[photos-owed] digest failed:', e.message) }
