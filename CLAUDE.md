...
- **NEVER delete `functions/adasiq-api/package-lock.json`**: deleting it unpins all ~200 backend packages at once. On 2026-07-09 this paired sharp's native binary (compiled against libvips 8.17.3) with a newer libvips (8.18.3) — the container crashed at module load, every request hung at the 30s gateway cap with only "Execution started" in the logs, and the app was down overnight. Recovery = `git checkout <good-commit> -- functions/adasiq-api/package-lock.json package.json`, then `npm ci --omit=dev --os=linux --cpu=x64 --libc=glibc --force` (Catalyst runs linux-x64; a plain install on a Mac uploads arm64 binaries that fail with `ERR_DLOPEN_FAILED`). Startup crash text appears as the HTTP response body of any endpoint — a fast-failing `/ping` with an error body means module-load crash; a pure 30s timeout means a hang.
- **Phone config is Datastore-backed** (`phone_config` table, since 2026-07-10): Phone Setup saves write Datastore first, cache second; `resolvePhoneConfig` self-heals cache from Datastore when the blob is missing. Don't regress this to cache-only — cache's 48h TTL is what silently wiped the Twilio credentials on 2026-07-09.
...
## Deploy gate (MANDATORY — added after the 2026-08-05 outage)
Run `./scripts/deploy-gate.sh` before EVERY `catalyst deploy` of the function. It syntax-checks all backend JS + verifies sharp is linux-x64. The 8/5 outage (7+ hours, login + webhooks down) was a mid-edit vanWeekly.js with a syntax error deployed from the shared working tree. `npm run deploy:staging` and `npm run deploy:function` now run the gate automatically — prefer them over bare catalyst commands. Multiple agent sessions share this repo; NEVER assume the working tree is clean — the gate is the check.
